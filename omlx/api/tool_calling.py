...
class _ToolCallPayloadPrefix:
    """Classify a generic ``<tool_call>`` payload before suppressing it.

    ``<tool_call>`` is also common in explanatory prose. Hold the envelope
    briefly until its payload has either established a recognized call shape
    or become impossible to parse as one. Structured but malformed calls
    remain suppressed: they are control markup, not user-facing prose.
    """
    ...
    def finish(self) -> bool:
        # An incomplete XML tag may be a truncated call; a bare word is prose.
        return self.decision if self.decision is not None else self._state == "tag"

    def is_registered_name(self) -> bool:
        return bool(self._tool_names) and "".join(self._name_parts) in self._tool_names
...
class ToolCallStreamFilter:
    ...
    def __init__(
        ...
        *,
        ...
        self._generic_prefix: Optional[_ToolCallPayloadPrefix] = None
    ...
    def _drain_buffer(self) -> str:
        ...
        while self._buffer:
            ...
            if self._suppressing_until is not None:
                if (
                    self._pending_start_marker == "<tool_call>"
                    and self._generic_prefix is not None
                ):
                    ...
                    if decision is False:
                        ...
                        self._buffer = (
                            "".join(self._pending_envelope_parts[1:])
                            + self._buffer
                        )
                        self._suppressing_until = None
            ...
            if start:
                ...
                if close_marker is not None:
                    ...
                    if close_marker != "__suppress_permanently__":
                        ...
                        if opening_marker == "<tool_call>":
                            self._generic_prefix = _ToolCallPayloadPrefix(
    ...
    def finish(self) -> str:
        ...
        recovered = ""
        if (
            self._pending_start_marker == _GENERIC_TOOL_CALL_START
            and self._generic_prefix is not None
            and self._registered_tool_names
            and self._generic_prefix.finish() is False
            and not self._generic_prefix.is_registered_name()
        ):
            candidate = "".join(self._pending_envelope_parts) + self._buffer
            start_marker = self._pending_start_marker or ""
            self._buffer = candidate[len(start_marker) :]
            self._suppressing_until = None
            self._clear_pending_envelope()
            recovered = start_marker + self._drain_buffer()
